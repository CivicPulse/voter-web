...
1. **Given** the user navigates to the elections section, **When** the page loads, **Then** a list of elections is displayed showing each election's name, date, type, status, and reporting progress (precincts reporting / participating).
...
3. **Given** elections are listed, **When** the user selects an election, **Then** they are navigated to the election results detail view for that election.
4. **Given** no elections match the active filters, **When** the user views the list, **Then** an appropriate empty state message is displayed.

---

### User Story 2 - View Election Results on a County Map (Priority: P1)

A user viewing an election's detail page sees a choropleth map of the state with counties colored to represent election data. The default map visualization shows vote share by leading candidate per county. The user can switch the map's data layer to show alternative metrics such as precincts reporting percentage or total votes cast per county. Hovering over a county shows a tooltip with key figures.
...
**Independent Test**: Can be fully tested by selecting an election, verifying the map renders with colored county overlays, hovering to confirm tooltips appear, and toggling map layers to confirm the visualization updates.

**Acceptance Scenarios**:

1. **Given** the user is on an election detail page with county results available, **When** the page loads, **Then** a choropleth map displays all counties colored by the leading candidate's party using a distinguishable color scheme.
2. **Given** the county map is displayed, **When** the user hovers over a county, **Then** a tooltip shows the county name, precincts reporting vs. participating, and each candidate's vote count and percentage.
...
4. **Given** the county map is displayed, **When** the user clicks on a county, **Then** the results drawer opens showing detailed results for that specific county.

---

### User Story 3 - Review Detailed Results in a Drawer (Priority: P1)

A user views detailed election result data in a bottom drawer panel (following the existing demographics drawer pattern). The drawer shows statewide summary results by default and can show county-specific results when a county is selected. Data includes candidate names, party affiliations, vote counts, vote percentages, and vote method breakdowns (e.g., Election Day, Absentee, Advance Voting).
...
**Independent Test**: Can be fully tested by opening the election results page, tapping the drawer trigger, verifying statewide results appear with all candidate data, then clicking a county on the map and verifying county-specific results appear in the drawer.

**Acceptance Scenarios**:

1. **Given** the user is on an election detail page, **When** the page loads, **Then** a bottom drawer trigger button is visible (matching the existing demographics drawer pattern).
2. **Given** the drawer trigger is visible, **When** the user opens the drawer, **Then** statewide results are displayed showing each candidate's name, party, vote count, vote percentage, and a visual bar indicating relative vote share.
3. **Given** the drawer is open showing statewide results, **When** vote method breakdowns are available, **Then** a collapsible section shows votes by method (e.g., Election Day, Absentee, Advance Voting) for each candidate.
4. **Given** a county is selected (via map click), **When** the drawer updates, **Then** it shows that county's specific results including county name, precincts reporting/participating, and per-candidate results.
...
A user wants more granular detail and switches to a precinct-level map view. The map displays precinct boundaries colored by leading candidate or reporting status. Because precinct data can be large, the user can filter by county to reduce the data load.
...
**Independent Test**: Can be fully tested by selecting an election, switching to precinct view, verifying precinct boundaries render, filtering by county, and confirming the filtered view loads correctly.

**Acceptance Scenarios**:

1. **Given** the user is on an election detail page, **When** they switch to the precinct map view, **Then** precinct boundaries are rendered on the map colored by leading candidate.
...
3. **Given** the precinct map is displayed, **When** the user hovers over a precinct, **Then** a tooltip shows the precinct identifier, reporting status, and candidate vote counts.
...
An admin user navigates to the admin elections management page and creates a new election by providing its name, date, type, district, data source URL, and optionally a refresh interval. A confirmation step is shown before the election is created, following the established two-step confirmation pattern used in other admin operations.
...
2. **Given** the admin elections list is displayed, **When** the admin clicks the "Create Election" button, **Then** a form is presented with fields for name, election date, election type (dropdown: general, primary, special, runoff), district, data source URL, and refresh interval (optional, defaults to 120 seconds, minimum 60 seconds).
...
- **Election**: A specific electoral event with a name, date, type (general/primary/special/runoff), status (active/finalized), district, data source URL, refresh interval, and reporting progress (precincts reporting vs. participating).
- **Candidate Result**: An individual candidate's outcome within an election, including name, party affiliation, ballot order, total vote count, and per-method vote breakdowns.
- **County Result**: Election results aggregated at the county level, including county name, local reporting progress (precincts reporting/participating), and per-candidate results within that county.
- **Precinct Result**: Election results at the individual precinct level, including geographic boundaries and per-candidate results (available via GeoJSON).
...
- The backend election API endpoints (list, detail, results, GeoJSON, create, update, refresh) are already implemented and available as documented at the `/api/v1/elections` path.
...
- **SC-002**: The county choropleth map renders with all county results visible within 3 seconds of opening an election detail page.
- **SC-003**: Users can identify the leading candidate in any county within 2 seconds by visual inspection of the map.
- **SC-004**: The results drawer displays complete statewide candidate results (names, parties, votes, percentages) in a single scroll view.
